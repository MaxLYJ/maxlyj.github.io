...
### Closed in iteration 34
- The iter-33-filed *latent* alt/title-fallback parity divergence is now **fixed** (no longer merely documented). When a project-detail block omits its explicit `alt`/`title`, the prerendered `<noscript>` (the copy crawlers / no-JS / AT readers see) used to fall back to a generic string (`""` / `"After"` / `"Before"` / `"Project video"`) while the runtime fell back to a descriptive, project-title-qualified one (`<Title> project detail image` / `<Title> after` / `<Title> before` / `<Title> project detail video`). No shipped block omits the explicit field, so this never bit a live page — but it was a real divergence-of-fallback in the same class as iter 28/32/33. A build-script-only change threads the project title into the prerender's `renderBlock` and mirrors the loader's fallbacks; the re-run is byte-identical (zero `*.html` churn — itself the proof 0 shipped blocks trigger a fallback), and a 16-assertion simulation confirms the latent case now matches the runtime exactly. The fix also closed a **4th sibling** the iter-33 enumeration missed (the video `title` fallback). The runtime↔prerender parity audit is now complete on every detail-block field — value, label, structure, AND fallback agree. See the iteration-34 section in [`CODE-REVIEW.md`](./CODE-REVIEW.md#-fixed-in-iteration-34-closed-the-iter-33-latent-alttitle-fallback-parity-divergence-p5-32-by-mirroring-the-loaders-title-qualified-fallbacks-in-the-build-time-prerender).

### Closed in iteration 35
- The homepage's social-preview image was the **185×185 author portrait** — below Facebook's 200×200 floor (Facebook rendered no preview image) and the wrong aspect for Twitter's large card — so sharing `maxlyj.com` (the most-shared URL) produced a broken/imageless card while the project pages (each shipping a 16:9 cover) were fine. Fixed by composing a dedicated **1200×630 branded card** from existing brand assets only (portrait avatar at native size, the logo, and the factual name + role already used as the page `<title>` — no new brand content), repointing `og:image` / `twitter:image` at it, and adding image dimensions + alt. (Content-adjacent: the card surfaces the author's name and role as the social-facing copy.) See the iteration-35 section in [`CODE-REVIEW.md`](./CODE-REVIEW.md#-fixed-in-iteration-35-seo--social-sharing-the-homepage-link-preview-image-was-a-185185-portrait--below-facebooks-200200-floor-so-sharing-maxlyjcom-rendered-no-preview-image).

### Closed in iteration 36
- The four project pages' social-preview images had **no dimension/type/alt metadata** (`og:image:type/width/height/alt` + `twitter:image:alt`) — the iter-35 fix added that cluster to the homepage only, leaving the project cards unable to be pre-laid-out by platforms and with no accessible image text. Fixed by extending the prerender's meta cluster from new measured `seo.imageType/imageWidth/imageHeight` fields + a derived alt (`` `${config.title} cover image` ``, matching the in-page cover image's alt). The iteration also ran the first **systematic image-asset audit** (downloading + measuring every referenced image): farcry6's cover (460×215) and raiden's (429×241) are upscaled ~3× at the hero (soft LCP), and division2's cover is 4K/771 KB served for ≤1280px — all confirmed **owner-gated** (no higher-res R2 siblings exist) and documented with a remediation plan, not worked around. See the iteration-36 section in [`CODE-REVIEW.md`](./CODE-REVIEW.md#-fixed-in-iteration-36-seo--social-sharing-parity-the-four-project-pages-emitted-only-ogimage--twitterimage-missing-the-dimensiontypealt-metadata-the-iter-35-homepage-got--platforms-couldnt-pre-layout-the-card-and-the-preview-image-had-no-accessible-text).
